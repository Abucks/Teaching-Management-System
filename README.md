...
**本地离线运行的教师工作台**：学生 / 排课 / 便签 / 成绩 / 学情，一个程序全搞定
...
![tests](https://img.shields.io/badge/tests-108%20逻辑%20%2B%2029%20界面%20%2B%20平台适配-brightgreen)
...
- 📋 **学情管理模块**（v1.4.0）：**自动读取学员名单**，用表格**自由补充**学生任何信息——可自行添加/重命名/删除字段（列），单元格随改随存，支持筛选与导出 Excel；🔴 **这些信息不会出现在学生管理的磁贴或气泡页中**（模块隔离）
- 🏫 **学生管理改为「班级总览 → 点入班级」两级浏览**（带滑动+视差过渡动画），磁贴**头像在上、完整姓名在头像正下方**，彻底解决姓名与图标重叠
- 🧭 **页面导航栏**：`← 上一页` + **可点击面包屑**（`🏠 班级总览 › 高一(1)班 › 👤 张三 › 🔍 搜索：张`），点任意一段即可跳回那一页
...
- 🗑 **单个 + 批量删除**：勾 1 个即单个删除，勾多个即批量；删除学生**自动级联清理其成绩与学情填写值**，删除便签自动清理图片文件
...
- ✅ **三套自动化测试**：108 项逻辑 + 29 项界面/新功能 + 平台与打包适配检查（含防「游离顶层窗口」与「学情模块隔离」回归）
...
| 成绩 · 统计图表 + 个人报告 | 学生资料 · 气泡标签云（v1.4.0） |
|:---:|:---:|
| ![](docs/screenshots/5_成绩_统计与个人报告.png) | ![](docs/screenshots/6_学生资料_信息气泡.png) |

| 学情管理 · 自由补充字段（v1.4.0） |
|:---:|
| ![](docs/screenshots/7_学情管理_自由填表.png) |
...
| 模块 | 关键操作 |
|---|---|
| 👥 **学生管理** | 首页是**班级磁贴**（人数 + 标签统计）→ 点入查看该班学生 → **单击学生磁贴看「气泡资料页」**（信息以大小不一的气泡呈现，可编辑/看成绩/删除）；顶部搜索可跨班级检索；导航栏可返回上一页或点面包屑跳转；磁贴悬停 ✏️/🗑 或右键编辑删除 |
...
| 📋 **学情管理** | **自动读取学员名单**；「➕ 添加字段」自建任意列（家庭情况/薄弱科目/家长电话…）；绿色单元格双击编辑**随改随存**；支持重命名/删除字段、关键词筛选、刷新名单、**导出 Excel**。⚠️ 此处信息**不会**出现在学生磁贴或气泡资料页 |
...
- 空单元格不会变成字符串「None」（v1.4.0 修复）；空白学号自动回退为学生姓名
